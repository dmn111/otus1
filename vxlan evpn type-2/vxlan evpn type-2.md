...
Построена модель overlay сети на оcнове технологии vxlan evpn type-2 на базе uderlay сети CLOS в рамках одного ЦОД.  Для моделирования  выбран один  L2 сегмент распределенный на три Leaf. В качестве конечных устройств выступают интрефейс SVI (ip -192.168.10.10/24)  коммутатора sw10 и   интерфейс e0/0 (ip -192.168.10.7/24) маршрутизатора R7. Для резервирования подключения коммутатора SW10:  на уровне L2 использована технолгия vPC ( mlag),  на уровне L3 резекрвирование шлюза технология anycast-gateway. В результате конфигруции оборудования достигнуты целевые показатели.
