...
В работе  рассматривается техноллогия объединения двух CLOS сетей по технологий VxLAN  Multipod . Каждый POD, с точки зрения протокола BGP  адресного прострастава  l2vp, находится в отдельной автономной системе ЦОД "A" в AS65001  и ЦОД  "B" в AS65002.   IP свзяность  между  всеми узлами обеспечивается за счет протокола  OSPF (в том числе между  POD в)  рамках одного инстанса.  Для обеспечения  корректного импорта/экспрота маршрутов  адресного протсрантства  l2vpn  находящихся в разных автономных ситстемах  используется ручное конфигурирование  route target.
